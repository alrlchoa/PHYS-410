...
def helper(A):
    ...
    bounce = solve_ivp(f, (ti,tf), y0=[y0,y0p],
                       dense_output=True, vectorized=True, max_step=1e2,rtol=1e-13, atol =1e-14)
    
    t = bounce.t
    y = bounce.y
    
    plt.figure()
    plt.plot(t, y[0], 'b-', linewidth=0.2)
    plt.title(r'$A = $'+str(A))
    plt.xlabel("t (sec)")
    plt.ylabel(r'$\theta$ (rad)')
    fname = fname = os.path.join('..','figs','q3_A_'+str(A)+'_no_wrap.pdf')
    plt.savefig(fname)
    ...
    for slc in unlink_wrap(y[0], [-np.pi, np.pi]):
        plt.plot(t[slc], y[0,slc], 'b-', linewidth=0.2)
    plt.title(r'$A = $'+str(A))
    plt.xlabel("t (sec)")
    plt.ylabel(r'$\theta$ (rad)')
    fname = fname = os.path.join('..','figs','q3_A_'+str(A)+'.pdf')
    plt.savefig(fname)
    
    plt.figure()
    for slc in unlink_wrap(y[0], [-np.pi, np.pi]):
        plt.plot(y[1][slc], y[0][slc], 'b-', linewidth=0.2)
    #plt.scatter(y[1,slc], y[0,slc], c='b', marker='x')
    plt.title(r'$A = $'+str(A)+" Phase portrait")
    plt.ylabel(r'$\theta$ (rad)')
    plt.xlabel(r'$v$ (rad/sec)')
    fname = fname = os.path.join('..','figs','q3_A_'+str(A)+'_phase.pdf')
    plt.savefig(fname)
...
if __name__ == '__main__':
    ...
    # 0.5 <= A <= 1.2
    A = [round(0.1*x,2) for x in range(20)]
